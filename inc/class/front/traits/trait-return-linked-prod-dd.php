...
if (!trait_exists('PBS_Return_Linked_Product_Dropdown')) :

    trait PBS_Return_Linked_Product_Dropdown {
         ...
         * @return void
         */
        public static function pbs_return_linked_product_dropdown($args = [], &$var_data = [], $prod, $current_curr) {
                        ...
                        <div class="attribute-swatch" attribute-index="">
                            <div class="swatchinput">
                                ...
                                foreach ($all_linked_products as $keyplugify => $valueplugify) {
                                    ...
                                    // retrieve variation data for product
                                    if ($product->is_type('variable')) :
                                        ...
                                        foreach ($av_vars as $index => $var_data) :
                                            $stock_variation_ids[] = [
                                                'variation_id'   => $var_data['variation_id'],
                                                'parent_id'      => $valueplugify[1],
                                                'attributes'     => $var_data['attributes'],
                                                'is_in_stock'    => $var_data['is_in_stock'],
                                                'is_purchasable' => $var_data['is_purchasable'],
                                                'availability'   => wc_get_product($var_data['variation_id'])->get_availability()
                                            ];
                                        endforeach;

                                        file_put_contents(PBS_Bundle_Path . 'variation_data.log', print_r($av_vars, true));
                                    ...
                                    if (!isset($var_data[$valueplugify[1]]) && $product->is_type('variable')) {
                                        ...
                                        foreach ($product->get_available_variations() as $key => $value) {
                                            array_push($var_arr, [
                                                ...
                                                'stock_status' => get_post_meta($value['variation_id'], '_stock_status', true)
                                            ]);
                                        }
                                ...
                                ?>
                                                    <div data-linked_id="<?php echo $valueplugify[1]; ?>" data-vars="<?php echo base64_encode(json_encode($stock_variation_ids)); ?>" class="imgclasssmall tooltipplugify selected" style="width:auto;border-radius: 2px;padding: 3px;border: 1px solid green;">
                                                    ...
                                                    ?>
                                                        ...
                                                        <label data-vars="<?php echo base64_encode(json_encode($stock_variation_ids)); ?>" class="wcvaswatchlabel wcvaround linked_product selected" id="linked_product_<?php echo $valueplugify[1] ?>" title="<?php echo filter_var($valueplugify[0]); ?>" data-attribute_name="attribute_pa_color" img-src="<?php echo get_the_post_thumbnail_url($valueplugify[1], 'single-post-thumbnail'); ?>" data-option="<?php echo filter_var($valueplugify[0]); ?>" data-linked_id="<?php echo $valueplugify[1] ?>" style="background-color:<?php echo sanitize_hex_color($attr_value); ?>; width:32px; height:32px; "></label>
                                                        <?php
                                                    } else {
                                                        if ($attr_img) {
                                                            $attr_image = '';
                                                        ...
                                                        ?>
                                                            <div data-linked_id="<?php echo $valueplugify[1]; ?>" data-vars="<?php echo base64_encode(json_encode($stock_variation_ids)); ?>" class="imgclasssmall tooltipplugify selected" style="margin: 0 5px;width:35px;height: 35px;border-radius: 50%;overflow: hidden;border: 1px solid green;">
                                                        ...
                                                        ?>
                                                            <div data-linked_id="<?php echo $valueplugify[1]; ?>" data-vars="<?php echo base64_encode(json_encode($stock_variation_ids)); ?>" class="imgclasssmall selected" style="width:auto; border-radius: 2px;padding: 3px;border: 1px solid green;">
                                                ...
                                                if ('true' == $istrue) {
                                                    ...
                                                    ?>
                                                    <div data-linked_id="<?php echo $valueplugify[1]; ?>" data-is-out-of-stock="<?php echo $is_out_of_stock ?>" id="linked_product_<?php echo $valueplugify[1] ?>" class="imgclasssmall tooltipplugify <?php echo (empty($style_cursor) ? '' : 'disabled') ?>" style="width:auto;border-radius: 2px;padding: 3px;border: 1px solid #ddd; <?php echo filter_var($style_cursor); ?>" img-src="<?php echo get_the_post_thumbnail_url($valueplugify[1], 'single-post-thumbnail'); ?>">
                                                    ...
                                                    ?>
                                                        ...
                                                        <label data-vars="<?php echo base64_encode(json_encode($stock_variation_ids)); ?>" id="linked_product_<?php echo $valueplugify[1] ?>" class="wcvaswatchlabel wcvaround linked_product <?php echo (empty($style_cursor) ? '' : 'disabled') ?>" title="<?php echo filter_var($valueplugify[0]); ?>" data-attribute_name="attribute_pa_color" img-src="<?php echo get_the_post_thumbnail_url($valueplugify[1], 'single-post-thumbnail'); ?>" data-option="<?php echo filter_var($valueplugify[0]); ?>" data-linked_id="<?php echo $valueplugify[1] ?>" style="background-color:<?php echo sanitize_hex_color($attr_value); ?>; width:32px; height:32px; <?php echo $style_cursor; ?>"></label>
                                                        ...
                                                        ?>
                                                            <div data-linked_id="<?php echo $valueplugify[1]; ?>" data-vars="<?php echo base64_encode(json_encode($stock_variation_ids)); ?>" id="linked_product_<?php echo $valueplugify[1] ?>" class="imgclasssmall tooltipplugify <?php echo (empty($style_cursor) ? '' : 'disabled') ?>" style="margin: 0 5px;width:35px;height:35px;border-radius: 50%;overflow: hidden;border: 1px solid #ddd; <?php echo filter_var($style_cursor); ?>" img-src="<?php echo get_the_post_thumbnail_url($valueplugify[1], 'single-post-thumbnail'); ?>">
                                                                <a style="<?php echo filter_var($style_cursor); ?>" <?php echo filter_var($htmllpluigg); ?> <?php echo filter_var($plgfqdp_ishyper); ?> class="aclass-clr" href="#"><img class="child_class_plugify" style="height: 35px;text-align: center;" src="<?php echo filter_var($attr_image); ?>">
                                                                </a>
                                                        ...
                                                        ?>
                                                            <div data-linked_id="<?php echo $valueplugify[1]; ?>" data-vars="<?php echo base64_encode(json_encode($stock_variation_ids)); ?>" class="imgclasssmall <?php echo (empty($style_cursor) ? '' : 'disabled') ?>" style="width:auto;border-radius: 2px;padding: 3px;border: 1px solid #ddd; background-color: <?php echo sanitize_hex_color($attr_value);
                                                                                                                                                                                                                                                                                                                        echo filter_var($style_cursor); ?>" img-src="<?php echo get_the_post_thumbnail_url($valueplugify[1], 'single-post-thumbnail'); ?>">
