jQuery(document).ready(function ($) {
    ...
    function i_bd_checkout(e) {
        ...
        var info = {};
        info['action'] = 'pbs_add_to_cart_multiple';
   ...
   */
    $('#pbs_bundle_atc').click(function (e) {

        e.preventDefault();

        if ($('#bd_checkout .item-selection.bd_active_product').length) {
            bd_update_item_cart_ajax(false, true);
        }
    });

    /**
     * Update cart on variation swatch click
     */
    $('.wcvaswatchlabel, .tooltipplugify').click(function (e) {
    ...
    /**
     * Update cart on variation dropdown change
     */
    ...
    // add to cart on package click/select
    $('.bd_c_package_option').click(function () { 
        setTimeout(() => {
            bd_update_item_cart_ajax(false, false);
        }, 250);
    });
    ...
    // disable toolplugify swatch link on click
    // $('.tooltipplugify > a').click(function (e) { 
    //     e.preventDefault();
    // });
